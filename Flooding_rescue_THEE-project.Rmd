...
## Generating multiple replicates

To calculate the probability of rescue
...
## Analyze data - find the means of the population sizes
...
plot(ratios$t_pause, ratios$Nw_survival_ratio, 
     ...
     col = "chocolate", 
     type = "b")  # Use a solid circle for points

# Add lines for Mutation survival ratio
lines(ratios$t_pause, ratios$Nm_survival_ratio, 
      col = "darkolivegreen4", 
      type = "b")  # Use points and lines

# Optionally, add a legend
legend("bottomright", legend = c("Wildtype", "Mutation"), 
       col = c("chocolate", "darkolivegreen4"), 
...
mean_comp_WT <- aggregate(data_table_comp$Proportion_WT ~ f, data = data_table_comp, FUN = mean)
mean_comp_Mut <- aggregate(data_table_comp$Proportion_Mut ~ f, data = data_table_comp, FUN = mean)
