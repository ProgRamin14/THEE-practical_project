...
### plotting a "forever event"

We want to simulate a "forever event" to see how fast the wildtype goes extinct on average. This will serve us as a starting point, as this value is the maximum duration the flooding-event can have. 

```{r}
simulate_pop <- function(N_init_a, N_init_A, decay_rate, sel_coeff, mut_rate, t_max, t_pause, t_event, t_switch) {
  # Create the vector in which to save the results
  pop_vector <- c(N_init_A, N_init_a)
  # initiate the variables
  pop_new <- c(N_init_A, N_init_a)
  
  ph = 0  # phase in the beginning of the simulation (even: no flood, odd: flood)
  
  # Track the extinction generation of the wildtype
  extinction_gen <- NA

  # Run the simulation until generation t_max
  for (i in 1:t_max) {
    # Determine whether a flood event or a pause is ongoing
    if(ph %% 2 == 0){  # No flood (even phase)
      decay_rate = -0.02  # "growth rate" of wildtype when no flood is present
      sel_coeff = -0.015  # selection coefficient of wildtype when no flood is present 
    } else {  # Flood is present (odd phase)
      decay_rate = 0.03   # decay rate of wildtype if flood is present
      sel_coeff = 0.05    # selection coefficient of mutant during flood
    }

    if (i >= t_switch) { # Check if we need to switch the phase
      if(ph %% 2 == 0) {  # After no flood, start flood
        t_switch = t_switch + t_event
        ph = ph + 1
      } else {  # After flood, pause for some time
    ...
    # Track extinction: if wildtype population reaches 0
    if (pop_new[1] <= 0 && is.na(extinction_gen)) {
      extinction_gen <- i  # Record the extinction generation
      break  # Stop the simulation once the wildtype is extinct
    }
    
    # Optional condition to stop if total population size exceeds some threshold
    if (pop_new[1] + pop_new[2] == 0) break
  }
  
  # Return the result: the population data and the extinction generation
  return(list(population_data = pop_vector, extinction_gen = extinction_gen))
}

# Function for running multiple simulations and averaging the extinction generations
run_multiple_simulations <- function(N_init_A, N_init_a, decay_rate, sel_coeff, mut_rate, t_max, t_pause, t_event, t_switch, n_simulations) {
  extinction_generations <- numeric(n_simulations)  # Vector to store extinction generations

  for (i in 1:n_simulations) {
    output_result <- simulate_pop(N_init_a, N_init_A, decay_rate, sel_coeff, mut_rate, t_max, t_pause, t_event, t_switch)
    extinction_generations[i] <- output_result$extinction_gen
  }

  # Calculate the average extinction generation, ignoring NA values (in case some simulations didn't result in extinction)
  avg_extinction_gen <- mean(extinction_generations, na.rm = TRUE)
  return(avg_extinction_gen)
}

# Define simulation parameters
max_gen <- 1000
n_simulations <- 1000  # Number of simulations
t_pause <- 1           # Time between floods
t_event <- 500         # Duration of the flood
t_switch <- t_pause    # Time to switch between flood and no flood
N_init_A <- 1000       # Initial wildtype population size
N_init_a <- 0          # Initial mutant population size (wildtype only in the start)

# Run multiple simulations and calculate the average extinction generation
avg_extinction_gen <- run_multiple_simulations(N_init_A, N_init_a, 0, -0.015, 0.001, max_gen, t_pause, t_event, t_switch, n_simulations)

# Print the average extinction generation
cat("Average extinction generation of wildtype over", n_simulations, "simulations:", avg_extinction_gen, "\n")
...
# Assuming frequencies is defined as a vector of frequencies, e.g.:  # Example, replace with your actual frequencies

# Example of data_table_Nw and data_table_Nm as data frames (replace with your actual data):
# data_table_Nw <- data.frame(frequency = c(0.1, 0.2, 0.3, 0.1, 0.2), size = c(100, 200, 300, 0, 250))
# data_table_Nm <- data.frame(frequency = c(0.1, 0.2, 0.3, 0.1, 0.2), size = c(120, 220, 320, 0, 270))
